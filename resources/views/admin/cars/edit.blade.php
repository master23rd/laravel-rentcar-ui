                    ...
                    <div class="card-body">
                        <form method="post" action="{{ route('admin.cars.update', $car) }}" enctype="multipart/form-data">
                            ...
                            <div class="form-group row border-bottom pb-4">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" class="form-control" name="name" value="{{ old('name', $car->name) }}" id="name">
                                </div>
                            </div>
                            <div class="form-group row border-bottom pb-4">
                                <label for="type_iid" class="col-sm-2 col-form-label">Brand</label>
                                <div class="col-sm-10">
                                    <select class="form-control" name="brand_id" id="brand_id">
                                        @foreach($brands as $brand)
                                        <option {{ old('type', $car->brands->id) == $brand->id ? 'selected' : null }} value="{{ $brand->id }}">{{ $brand->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="form-group row border-bottom pb-4">
                                <label for="type_iid" class="col-sm-2 col-form-label">Model</label>
                                <div class="col-sm-10">
                                    <select class="form-control" name="model_id" id="model_id">
                                        @foreach($models as $model)
                                        <option {{ old('type', $car->models->id) == $model->id ? 'selected' : null }} value="{{ $model->id }}">{{ $model->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="form-group row border-bottom pb-4">
                                ...
                                <div class="col-sm-10">
                                    <input type="number" min='1' class="form-control" name="doors" value="{{ old('doors', $car->doors) }}" id="doors">
                                </div>
                            </div>
                            <div class="form-group row border-bottom pb-4">
                                <label for="penumpang" class="col-sm-2 col-form-label">Jumlah Penumpang</label>
                                <div class="col-sm-10">
                                    <input type="number" min='1' class="form-control" name="passengers" value="{{ old('passengers', $car->passengers) }}" id="passengers">
                                </div>
                            </div>
                            <div class="form-group row border-bottom pb-4">
                            ...
                            <div class="form-group row border-bottom pb-4">
                                ...
                                <div class="col-sm-10">
                                    <select class="form-control" name="status" id="status">
                                        ...
                                        <option {{ old('status', $car->status) == $no ? 'selected' : null }} value="{{ $no }}">{{ $status }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
